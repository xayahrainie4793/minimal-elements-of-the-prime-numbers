...
// code base mxlg
// tries to extend the current kernel for base with primes up to length mxlg, while starting with the families provided on the standard input file
 ...
 while(cin>>p)
  {f.clear();ss="";
   for(i=0;i<p.length();i++)
      {if(p[i]=='{'||p[i]=='}'){f.push_back(ss);ss="";} else ss=ss+p[i];}
   f.push_back(ss);
//construction of the familles to be explored 
   handelf(f);
  }
